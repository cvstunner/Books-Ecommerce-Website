...
function populateProducts(data) {
    ...
    data.forEach((value, index) => {
        ...
        pro_frow.innerHTML += '<div class="pro-wrpr" data-cartId=' + value['wishlistId'] + ' ><div class="pro-buy-cont pro-buy-cont-af"><div class="pro-i-wrapper pro-i-wrapperaf"><div class="pro-i pro-frow-i1 pro-i-buy" style="background-image: url(\'userData/images/' + value['filename'] + '\');"></div></div></div><div class="pro-opt pro-opt-af"><div class="pro-i-det pro-i-detaf"><div><p class="pro-name">' + value['title'] + '</p><p class="pro-author">' + value['author'] + '</p><p class="pro-author">cvstunner PVT LTD</p><div class="pro-prices"><p class="pro-mrp-price">' + value['MRP_price'] + '</p><p class="pro-rel-price pro-rel-priceaf">₹' + value['retail_price'] + '</p><button id="cart-pro-rmv-btn" class="pro-btns pro-adtocrt-btn deliver-clr-btn" type="button" onclick="cart_pro_rmv_btn_clicked(this)">Remove</button></div></div></div></div></div>';
    });
}

function populateCart(element){
    let query = 'select wishlist.userId, wishlist.wishlistId, products.title, products.author, products.MRP_price, products.retail_price, book_filename.filename from wishlist INNER JOIN products ON wishlist.productId=products.productId JOIN book_filename ON products.fileId = book_filename.fileId where wishlist.userId = ?';
    queryMe('query/addWishlist',query, [window.sessionStorage.getItem('userId')], function(response){
        if (response[0] === true){
